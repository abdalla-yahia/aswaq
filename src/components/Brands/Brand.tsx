...
export default function Brand({ brand }: { brand: string }) {
   ...
   * and filter them based on the selected brand
  ...
  const ProductsOfBrand = products?.data?.filter((product:CreateProductType) => product?.brand?.name === brand)
  ...
  return (
    <section className="Brand w-full py-8 flex flex-col justify-start items-start gap-0">
      ...
      <h1 className="mb-5 text-3xl flex justify-center items-center">قائمة منتجات
        ...
        <Image src={products?.[0]?.brand?.logo} alt={brand} width={40} height={40} />
      </h1>
      {/**Page Container */}
      <div className="flex w-full justify-between items-start gap-5">
        ...
        <div className="flex flex-col justify-center items-start gap-2 w-4/6 md:w-5/6">
            {/**Banner Image*/}
          <div style={{backgroundImage:`url(${images?.data[randomNumber]?.image})`}} className="w-full bg-cover  h-[100px] md:h-[200px] mb-4 bg-white/70 rounded-lg shadow-md">
            <Image src={products?.[0]?.brand?.logo} alt={brand} width={100} height={100} className="w-full  h-[100px] md:h-[200px]"/>
          </div>
          {/**Products */}
